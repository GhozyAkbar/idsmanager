...
/* 
 CanvasJS Chart - v3.12.0 GA - https://canvasjs.com/ 
...
"undefined"!==e.x5&&(a.lineTo(Math.round(e.x5),Math.round(e.y5)),a.lineTo(Math.round(e.x6),Math.round(e.y6))),a.closePath(),a.fillStyle=b?b:e.color,a.fill(),a.globalAplha=1,a.restore())};x.prototype.renderFunnel=function(a){function e(){for(var a=0,b=[],c=0;c<z.length;c++){if("undefined"===typeof z[c].y)return-1;z[c].y="number"===typeof z[c].y?z[c].y:0;a+=Math.abs(z[c].y)}if(0===a)return-1;for(c=b[0]=0;c<z.length;c++)b.push(Math.abs(z[c].y)*F/a);return b}function c(){var a=Y,b=aa,c=T,d=ba,e,g;e=S;
g=U-N;d=Math.abs((g-e)*(b-a+(d-c))/2);c=ba-T;e=g-e;g=c*(g-U);g=Math.abs(g);g=d+g;for(var d=[],h=0,f=0;f<z.length;f++){if("undefined"===typeof z[f].y)return-1;z[f].y="number"===typeof z[f].y?z[f].y:0;h+=Math.abs(z[f].y)}if(0===h)return-1;for(var q=d[0]=0,k=0,l,m,b=b-a,q=!1,f=0;f<z.length;f++)a=Math.abs(z[f].y)*g/h,q?l=0==Number(c.toFixed(3))?0:a/c:(m=ea*ea*b*b-4*Math.abs(ea)*a,0>m?(m=c,q=(b+m)*(e-k)/2,a-=q,l=e-k,k+=e-k,l+=0==m?0:a/m,k+=a/m,q=!0):(l=(Math.abs(ea)*b-Math.sqrt(m))/2,m=b-2*l/Math.abs(ea),
k+=l,k>e&&(k-=l,m=c,q=(b+m)*(e-k)/2,a-=q,l=e-k,k+=e-k,l+=a/m,k+=a/m,q=!0),b=m)),d.push(l);return d}function b(){if(s&&z){for(var a,b,c,e,d,h,f,k,q,m,n,p,u,v,w,E,t,x,C=[],A=[],B={percent:null,total:null},G=null,D=0;D<z.length;D++)x=P[D],x="undefined"!==typeof x.x5?(x.y2+x.y4)/2:(x.y2+x.y3)/2,x=g(x).x2+1,C[D]=Q-(isNaN(x)?0:x)-V;x=0.5*V;for(var D=0,J=z.length-1;D<z.length||0<=J;D++,J--){b=s.reversed?z[J]:z[D];a=b.color?b.color:s.reversed?s._colorSet[(z.length-1-D)%s._colorSet.length]:s._colorSet[D%s._colorSet.length];
...
u,a[g].lineCoordinates.y2=u,"inside"===a[g].labelPlacement&&0<g&&(a[g].lineCoordinates.y1=a[0].lineCoordinates.y2+D+(a[g].labelEffectiveHeight||0),a[g].lineCoordinates.y2=a[g].lineCoordinates.y1+a[g].lineThickness/2),"inside"===a[g].tickPlacement&&0<g&&(a[g].lineCoordinates.y1+=a[g].tickLength,a[g].lineCoordinates.y2=a[g].lineCoordinates.y1+a[g].lineThickness/2),a[g].bounds={x1:f,y1:u,x2:r,y2:k-(X+d-z[g]-D),width:r-f},a[g].bounds.height=a[g].bounds.y2-a[g].bounds.y1),D+=z[g]+a[g].margin;if(e&&0<e.length)for(g=
0;g<e.length;g++)e[g].lineCoordinates.x1=Math.round(h.x1+P+T),e[g].lineCoordinates.x2=r?r:Math.round(h.x2-ba-aa>m.width-n?m.width-n:h.x2-ba-aa),e[g].lineCoordinates.width=Math.abs(r-f),e[g]._labels&&1<e[g]._labels.length&&(k=e[g]._labels[1],u="dateTime"===e[g].valueType?e[g]._labels[e[g]._labels.length-2]:e[g]._labels[e[g]._labels.length-1],x=k.textBlock.width*Math.cos(Math.PI/180*Math.abs(k.textBlock.angle))+(k.textBlock.height-u.textBlock.fontSize/2)*Math.sin(Math.PI/180*Math.abs(k.textBlock.angle)),
s=u.textBlock.width*Math.cos(Math.PI/180*Math.abs(u.textBlock.angle))+(u.textBlock.height-u.textBlock.fontSize/2)*Math.sin(Math.PI/180*Math.abs(u.textBlock.angle))),m.panEnabled?$=l(m.sessionVariables.axisX2.height)||m.stockChart?m.sessionVariables.axisX2.height=$:m.sessionVariables.axisX2.height:m.sessionVariables.axisX2.height=$,u=Math.round(h.y1),k=e[g].lineCoordinates.y1=u+$+Y-U,e[g].lineCoordinates.y2=u,"inside"===e[g].labelPlacement&&0<g&&(e[g].lineCoordinates.y1=e[0].lineCoordinates.y1-U-(e[g].labelEffectiveHeight||
0)-5),"inside"===e[g].tickPlacement&&0<g&&(e[g].lineCoordinates.y1-=e[g].tickLength),e[g].bounds={x1:f,y1:u+($+Y-("inside"===e[g].labelPlacement&&0===g?v[g]-5:v[g])-U),x2:r,y2:k,width:r-f},e[g].bounds.height=e[g].bounds.y2-e[g].bounds.y1,U+=("inside"===e[g].labelPlacement&&0===g?v[g]-5:v[g])+e[g].margin;if(c&&0<c.length)for(g=0;g<c.length;g++)T=m.isNavigator?0:10,c[g]&&(f=Math.round(a[0]?a[0].lineCoordinates.x1:e[0].lineCoordinates.x1),T=c[g]._labels&&0<c[g]._labels.length?c[g]._labels[c[g]._labels.length-
1].textBlock.height/2:n,u=Math.round(h.y1+$+Y<Math.max(T,n)?Math.max(T,n):h.y1+$+Y),r=Math.round(a[0]?a[0].lineCoordinates.x1:e[0].lineCoordinates.x1),T=0<a.length?0:c[g]._labels&&0<c[g]._labels.length?c[g]._labels[0].textBlock.height/2:n,k=Math.round(h.y2-X-d-T),c[g].lineCoordinates={x1:f-V,y1:u,x2:r-V,y2:k,height:Math.abs(k-u)},"inside"===c[g].labelPlacement&&0<g&&(c[g].lineCoordinates.x1=c[0].lineCoordinates.x1-V-(c[g].labelEffectiveWidth||0)-5,c[g].lineCoordinates.x2=c[g].lineCoordinates.x1+c[g].lineThickness/
2),"inside"===c[g].tickPlacement&&0<g&&(c[g].lineCoordinates.x1-=c[g].tickLength,c[g].lineCoordinates.x2=c[g].lineCoordinates.x1+c[g].lineThickness/2),c[g].bounds={x1:f-(("inside"===c[g].labelPlacement&&0===g?A[g]-5:A[g])+V),y1:u,x2:r-V,y2:k,height:k-u},c[g].bounds.width=c[g].bounds.x2-c[g].bounds.x1,c[g].title&&(c[g]._titleTextBlock.maxWidth=0<c[g].titleMaxWidth&&c[g].titleMaxWidth<c[g].lineCoordinates.height?c[g].titleMaxWidth:c[g].lineCoordinates.height),V+=("inside"===c[g].labelPlacement&&0===
g?A[g]-5:A[g])+c[g].margin);if(b&&0<b.length)for(g=0;g<b.length;g++)b[g]&&(f=Math.round(a[0]?a[0].lineCoordinates.x2:e[0].lineCoordinates.x2),r=Math.round(f),T=b[g]._labels&&0<b[g]._labels.length?b[g]._labels[b[g]._labels.length-1].textBlock.height/2:0,u=Math.round(h.y1+$+Y<Math.max(T,n)?Math.max(T,n):h.y1+$+Y),T=0<a.length?0:b[g]._labels&&0<b[g]._labels.length?b[g]._labels[0].textBlock.height/2:0,k=Math.round(h.y2-(X+d+T)),b[g].lineCoordinates={x1:f+W,y1:u,x2:f+W,y2:k,height:Math.abs(k-u)},"inside"===
b[g].labelPlacement&&0<g&&(b[g].lineCoordinates.x1=b[0].lineCoordinates.x2+W+(b[g].labelEffectiveWidth||0)+5,b[g].lineCoordinates.x2=b[g].lineCoordinates.x1+b[g].lineThickness/2),"inside"===b[g].tickPlacement&&0<g&&(b[g].lineCoordinates.x1+=b[g].tickLength,b[g].lineCoordinates.x2=b[g].lineCoordinates.x1+b[g].lineThickness/2),b[g].bounds={x1:f+W,y1:u,x2:r+(("inside"===b[g].labelPlacement&&0===g?B[g]-5:B[g])+W),y2:k,height:k-u},b[g].bounds.width=b[g].bounds.x2-b[g].bounds.x1,b[g].title&&(b[g]._titleTextBlock.maxWidth=
0<b[g].titleMaxWidth&&b[g].titleMaxWidth<b[g].lineCoordinates.height?b[g].titleMaxWidth:b[g].lineCoordinates.height),W+=("inside"===b[g].labelPlacement&&0===g?B[g]-5:B[g])+b[g].margin);if(a&&0<a.length)for(g=0;g<a.length;g++)a[g]&&(a[g].calculateValueToPixelConversionParameters(),a[g].calculateBreaksSizeInValues(),a[g]._labels&&1<a[g]._labels.length&&(F=(a[g].logarithmic?Math.log(a[g]._labels[1].position/a[g].viewportMinimum)/a[g].conversionParameters.lnLogarithmBase:a[g]._labels[1].position-a[g].viewportMinimum)*
Math.abs(a[g].conversionParameters.pixelPerUnit)+a[g].lineCoordinates.x1,f=a[g]._labels[a[g]._labels.length-("dateTime"===a[g].valueType?2:1)].position,f=a[g].getApparentDifference(a[g].viewportMinimum,f),I=a[g].logarithmic?(1<f?Math.log(f)/a[g].conversionParameters.lnLogarithmBase*Math.abs(a[g].conversionParameters.pixelPerUnit):0)+a[g].lineCoordinates.x1:(0<f?f*Math.abs(a[g].conversionParameters.pixelPerUnit):0)+a[g].lineCoordinates.x1));if(e&&0<e.length)for(g=0;g<e.length;g++)e[g].calculateValueToPixelConversionParameters(),
e[g].calculateBreaksSizeInValues(),e[g]._labels&&1<e[g]._labels.length&&(F=(e[g].logarithmic?Math.log(e[g]._labels[1].position/e[g].viewportMinimum)/e[g].conversionParameters.lnLogarithmBase:e[g]._labels[1].position-e[g].viewportMinimum)*Math.abs(e[g].conversionParameters.pixelPerUnit)+e[g].lineCoordinates.x1,f=e[g]._labels[e[g]._labels.length-("dateTime"===e[g].valueType?2:1)].position,f=e[g].getApparentDifference(e[g].viewportMinimum,f),I=e[g].logarithmic?(1<f?Math.log(f)/e[g].conversionParameters.lnLogarithmBase*
Math.abs(e[g].conversionParameters.pixelPerUnit):0)+e[g].lineCoordinates.x1:(0<f?f*Math.abs(e[g].conversionParameters.pixelPerUnit):0)+e[g].lineCoordinates.x1);for(g=0;g<p.length;g++)"axisY"===p[g].type&&(p[g].calculateValueToPixelConversionParameters(),p[g].calculateBreaksSizeInValues());if(0<w){if(a&&0<a.length)for(g=0;g<a.length;g++)t=R[w-1][g]===R[w][g]?!0:!1;else t=!0;if(e&&0<e.length)for(g=0;g<e.length;g++)L=S[w-1][g]===S[w][g]?!0:!1;else L=!0;if(c&&0<c.length)for(g=0;g<c.length;g++)M=O[w-1][g]===
O[w][g]?!0:!1;else M=!0;if(b&&0<b.length)for(g=0;g<b.length;g++)N=Q[w-1][g]===Q[w][g]?!0:!1;else N=!0}if(t&&L&&M&&N)break;w++}if(a&&0<a.length)for(g=0;g<a.length;g++)a[g].calculateStripLinesThicknessInValues(),a[g].calculateBreaksInPixels();if(e&&0<e.length)for(g=0;g<e.length;g++)e[g].calculateStripLinesThicknessInValues(),e[g].calculateBreaksInPixels();if(c&&0<c.length)for(g=0;g<c.length;g++)c[g].calculateStripLinesThicknessInValues(),c[g].calculateBreaksInPixels();if(b&&0<b.length)for(g=0;g<b.length;g++)b[g].calculateStripLinesThicknessInValues(),
b[g].calculateBreaksInPixels()}else{n=[];F=[];K=[];x=[];I=[];s=[];R=[];for(S=[];4>w;){X=W=V=aa=T=Y=d=Q=O=D=$=0;if(a&&0<a.length)for(K=[],g=W=0;g<a.length;g++)K.push(Math.ceil(a[g]?a[g].createLabelsAndCalculateWidth():0)),W+=K[g],d+=a[g]&&!m.isNavigator?a[g].margin:0;else K.push(Math.ceil(a[0]?a[0].createLabelsAndCalculateWidth():0));R.push(K);if(e&&0<e.length)for(x=[],g=X=0;g<e.length;g++)x.push(Math.ceil(e[g]?e[g].createLabelsAndCalculateWidth():0)),X+=x[g],Y+=e[g]?e[g].margin:0;else x.push(Math.ceil(e[0]?
e[0].createLabelsAndCalculateWidth():0));S.push(x);if(c&&0<c.length)for(g=0;g<c.length;g++)c[g].lineCoordinates={},f=Math.round(h.x1+W+d),r=Math.round(h.x2-X-Y>m.width-10?m.width-10:h.x2-X-Y),c[g].labelAutoFit&&!l(z)&&(0<!a.length&&(f=0>c[g].labelAngle?Math.max(f,z):0===c[g].labelAngle?Math.max(f,z/2):f),0<!e.length&&(r=0<c[g].labelAngle?r-v/2:0===c[g].labelAngle?r-v/2:r)),c[g].lineCoordinates.x1=f,c[g].lineCoordinates.x2=r,c[g].lineCoordinates.width=Math.abs(r-f),c[g].title&&(c[g]._titleTextBlock.maxWidth=
0<c[g].titleMaxWidth&&c[g].titleMaxWidth<c[g].lineCoordinates.width?c[g].titleMaxWidth:c[g].lineCoordinates.width);if(b&&0<b.length)for(g=0;g<b.length;g++)b[g].lineCoordinates={},f=Math.round(h.x1+W+d),r=Math.round(h.x2-X-Y>b[g].chart.width-10?b[g].chart.width-10:h.x2-X-Y),b[g]&&b[g].labelAutoFit&&!l(A)&&(0<!a.length&&(f=0<b[g].labelAngle?Math.max(f,A):0===b[g].labelAngle?Math.max(f,A/2):f),0<!e.length&&(r-=B/2)),b[g].lineCoordinates.x1=f,b[g].lineCoordinates.x2=r,b[g].lineCoordinates.width=Math.abs(r-
f),b[g].title&&(b[g]._titleTextBlock.maxWidth=0<b[g].titleMaxWidth&&b[g].titleMaxWidth<b[g].lineCoordinates.width?b[g].titleMaxWidth:b[g].lineCoordinates.width);if(c&&0<c.length)for(n=[],g=V=0;g<c.length;g++)n.push(Math.ceil(c[g]?c[g].createLabelsAndCalculateHeight():0)),V+=n[g]+c[g].margin,T+=c[g].margin;else n.push(Math.ceil(c[0]?c[0].createLabelsAndCalculateHeight():0));I.push(n);if(b&&0<b.length)for(F=[],g=0;g<b.length;g++)F.push(Math.ceil(b[g]?b[g].createLabelsAndCalculateHeight():0)),aa+=b[g].margin;
...
m.sessionVariables.axisY[g].height=n[g]:m.sessionVariables.axisY[g].height;else for(g=0;g<c.length;g++)m.sessionVariables.axisY[g].height=n[g];if(c&&0<c.length)for(g=c.length-1;0<=g;g--)u=Math.round(h.y2),k=Math.round(h.y2>c[g].chart.height?c[g].chart.height:h.y2),c[g].lineCoordinates.y1=u-(n[g]+c[g].margin+$),c[g].lineCoordinates.y2=u-(n[g]+c[g].margin+$),"inside"===c[g].labelPlacement&&0<g&&(c[g].lineCoordinates.y1=c[g].lineCoordinates.y1+n[g]-(c[g]._titleTextBlock?c[g]._titleTextBlock.height:0)-
c[g].tickLength-(c[g].stripLineLabelEffectiveHeight||0)-5,c[g].lineCoordinates.y2=c[g].lineCoordinates.y1+c[g].lineThickness/2),"inside"===c[g].tickPlacement&&0<g&&(c[g].lineCoordinates.y1+=c[g].tickLength,c[g].lineCoordinates.y2=c[g].lineCoordinates.y1+c[g].lineThickness/2),c[g].bounds={x1:f,y1:u-(n[g]+$+c[g].margin),x2:r,y2:k-($+c[g].margin),width:r-f,height:n[g]},c[g].title&&(c[g]._titleTextBlock.maxWidth=0<c[g].titleMaxWidth&&c[g].titleMaxWidth<c[g].lineCoordinates.width?c[g].titleMaxWidth:c[g].lineCoordinates.width),
$+=n[g]+c[g].margin;if(b&&0<b.length)for(g=b.length-1;0<=g;g--)b[g]&&(u=Math.round(h.y1),k=Math.round(h.y1+(F[g]+b[g].margin+D)),b[g].lineCoordinates.y1=k,b[g].lineCoordinates.y2=k,"inside"===b[g].labelPlacement&&0<g&&(b[g].lineCoordinates.y1=k-F[g]+(b[g]._titleTextBlock?b[g]._titleTextBlock.height:0)+b[g].tickLength+(b[g].stripLineLabelEffectiveHeight||0),b[g].lineCoordinates.y2=b[g].lineCoordinates.y1-b[g].lineThickness/2),"inside"===b[g].tickPlacement&&0<g&&(b[g].lineCoordinates.y1-=b[g].tickLength,
b[g].lineCoordinates.y2=b[g].lineCoordinates.y1-b[g].lineThickness/2),b[g].bounds={x1:f,y1:u+(b[g].margin+D),x2:r,y2:k,width:r-f},b[g].bounds.height=b[g].bounds.y2-b[g].bounds.y1,b[g].title&&(b[g]._titleTextBlock.maxWidth=0<b[g].titleMaxWidth&&b[g].titleMaxWidth<b[g].lineCoordinates.width?b[g].titleMaxWidth:b[g].lineCoordinates.width),D+=F[g]+b[g].margin);if(a&&0<a.length)for(g=0;g<a.length;g++){T=a[g]._labels&&0<a[g]._labels.length?a[g]._labels[0].textBlock.fontSize/2:0;f=Math.round(h.x1+d);u=b&&
0<b.length?Math.round(b[0]?b[0].lineCoordinates.y2:h.y1<Math.max(T,10)?Math.max(T,10):h.y1):h.y1<Math.max(T,10)?Math.max(T,10):h.y1;r=Math.round(h.x1+W+d);k=c&&0<c.length?Math.round(c[0]?c[0].lineCoordinates.y1:h.y2-V>m.height-Math.max(T,10)?m.height-Math.max(T,10):h.y2-V):h.y2>m.height-Math.max(T,10)?m.height-Math.max(T,10):h.y2;if(c&&0<c.length)for(T=0;T<c.length;T++)c[T]&&c[T].labelAutoFit&&(r=c[T].lineCoordinates.x1,f=0>c[T].labelAngle||0===c[T].labelAngle?r-W:f);if(b&&0<b.length)for(T=0;T<b.length;T++)b[T]&&
b[T].labelAutoFit&&(r=b[T].lineCoordinates.x1,f=r-W);a[g].lineCoordinates={x1:r-O,y1:u,x2:r-O,y2:k,height:Math.abs(k-u)};"inside"===a[g].labelPlacement&&0<g&&(a[g].lineCoordinates.x1=a[g].lineCoordinates.x1-(K[g]-(a[g]._titleTextBlock?a[g]._titleTextBlock.height:0))+a[g].tickLength+(a[g].stripLineLabelEffectiveWidth||0),a[g].lineCoordinates.x2=a[g].lineCoordinates.x1+a[g].lineThickness/2);"inside"===a[g].tickPlacement&&0<g&&(a[g].lineCoordinates.x1-=a[g].tickLength,a[g].lineCoordinates.x2=a[g].lineCoordinates.x1+
a[g].lineThickness/2);a[g].bounds={x1:r-(K[g]+O),y1:u,x2:r-O,y2:k,height:k-u};a[g].bounds.width=a[g].bounds.x2-a[g].bounds.x1;a[g].title&&(a[g]._titleTextBlock.maxWidth=0<a[g].titleMaxWidth&&a[g].titleMaxWidth<a[g].lineCoordinates.height?a[g].titleMaxWidth:a[g].lineCoordinates.height);a[g].calculateValueToPixelConversionParameters();a[g].calculateBreaksSizeInValues();O+=K[g]+a[g].margin}if(e&&0<e.length)for(g=0;g<e.length;g++){T=e[g]._labels&&0<e[g]._labels.length?e[g]._labels[0].textBlock.fontSize/
2:0;f=Math.round(h.x1-d);u=b&&0<b.length?Math.round(b[0]?b[0].lineCoordinates.y2:h.y1<Math.max(T,10)?Math.max(T,10):h.y1):h.y1<Math.max(T,10)?Math.max(T,10):h.y1;r=Math.round(h.x2-X-Y);k=c&&0<c.length?Math.round(c[0]?c[0].lineCoordinates.y1:h.y2-V>m.height-Math.max(T,10)?m.height-Math.max(T,10):h.y2-V):h.y2>m.height-Math.max(T,10)?m.height-Math.max(T,10):h.y2;if(c&&0<c.length)for(T=0;T<c.length;T++)c[T]&&c[T].labelAutoFit&&(r=0>c[T].labelAngle?Math.max(r,z):0===c[T].labelAngle?Math.max(r,z/2):r,f=
0>c[T].labelAngle||0===c[T].labelAngle?r-X:f);if(b&&0<b.length)for(T=0;T<b.length;T++)b[T]&&b[T].labelAutoFit&&(r=b[T].lineCoordinates.x2,f=r-X);e[g].lineCoordinates={x1:r+Q,y1:u,x2:r+Q,y2:k,height:Math.abs(k-u)};"inside"===e[g].labelPlacement&&0<g&&(e[g].lineCoordinates.x1=e[g].lineCoordinates.x1+(x[g]-(e[g]._titleTextBlock?e[g]._titleTextBlock.height:0)-2)-e[g].tickLength-(e[g].stripLineLabelEffectiveWidth||0),e[g].lineCoordinates.x2=e[g].lineCoordinates.x1+e[g].lineThickness/2);"inside"===e[g].tickPlacement&&
0<g&&(e[g].lineCoordinates.x1+=e[g].tickLength,e[g].lineCoordinates.x2=e[g].lineCoordinates.x1+e[g].lineThickness/2);e[g].bounds={x1:e[g].lineCoordinates.x1,y1:u,x2:r+x[g]+Q,y2:k,width:r-f,height:k-u};e[g].bounds.width=e[g].bounds.x2-e[g].bounds.x1;e[g].title&&(e[g]._titleTextBlock.maxWidth=0<e[g].titleMaxWidth&&e[g].titleMaxWidth<e[g].lineCoordinates.height?e[g].titleMaxWidth:e[g].lineCoordinates.height);e[g].calculateValueToPixelConversionParameters();e[g].calculateBreaksSizeInValues();Q+=x[g]+
e[g].margin}for(g=0;g<p.length;g++)"axisY"===p[g].type&&(p[g].calculateValueToPixelConversionParameters(),p[g].calculateBreaksSizeInValues());if(0<w){if(a&&0<a.length)for(g=0;g<a.length;g++)t=R[w-1][g]===R[w][g]?!0:!1;else t=!0;if(e&&0<e.length)for(g=0;g<e.length;g++)L=S[w-1][g]===S[w][g]?!0:!1;else L=!0;if(c&&0<c.length)for(g=0;g<c.length;g++)M=I[w-1][g]===I[w][g]?!0:!1;else M=!0;if(b&&0<b.length)for(g=0;g<b.length;g++)N=s[w-1][g]===s[w][g]?!0:!1;else N=!0}if(t&&L&&M&&N)break;w++}if(c&&0<c.length)for(g=
0;g<c.length;g++)c[g].calculateStripLinesThicknessInValues(),c[g].calculateBreaksInPixels();if(b&&0<b.length)for(g=0;g<b.length;g++)b[g].calculateStripLinesThicknessInValues(),b[g].calculateBreaksInPixels();if(a&&0<a.length)for(g=0;g<a.length;g++)a[g].calculateStripLinesThicknessInValues(),a[g].calculateBreaksInPixels();if(e&&0<e.length)for(g=0;g<e.length;g++)e[g].calculateStripLinesThicknessInValues(),e[g].calculateBreaksInPixels()}};B.render=function(a,e,c,b,d){d=a[0]?a[0].chart:e[0].chart;var h=
d.ctx;d.alignVerticalAxes&&d.alignVerticalAxes();h.save();h.beginPath();a&&a.length&&h.rect(2,a[0].bounds.y1,a[0].chart.width-4,a[a.length-1].bounds.y2);e&&e.length&&h.rect(2,e[e.length-1].bounds.y1,e[0].chart.width-4,e[0].bounds.y2);h.clip();if(a&&0<a.length)for(var f=0;f<a.length;f++)a[f].renderLabelsTicksAndTitle();if(e&&0<e.length)for(f=0;f<e.length;f++)e[f].renderLabelsTicksAndTitle();h.restore();if(c&&0<c.length)for(f=0;f<c.length;f++)c[f].renderLabelsTicksAndTitle();if(b&&0<b.length)for(f=
0;f<b.length;f++)b[f].renderLabelsTicksAndTitle();d.preparePlotArea();f=d.plotArea;h.save();h.beginPath();h.rect(f.x1,f.y1,Math.abs(f.x2-f.x1),Math.abs(f.y2-f.y1));h.clip();if(a&&0<a.length)for(f=0;f<a.length;f++)a[f].renderStripLinesOfThicknessType("value");if(e&&0<e.length)for(f=0;f<e.length;f++)e[f].renderStripLinesOfThicknessType("value");if(c&&0<c.length)for(f=0;f<c.length;f++)c[f].renderStripLinesOfThicknessType("value");if(b&&0<b.length)for(f=0;f<b.length;f++)b[f].renderStripLinesOfThicknessType("value");
if(a&&0<a.length)for(f=0;f<a.length;f++)a[f].renderInterlacedColors();if(e&&0<e.length)for(f=0;f<e.length;f++)e[f].renderInterlacedColors();if(c&&0<c.length)for(f=0;f<c.length;f++)c[f].renderInterlacedColors();if(b&&0<b.length)for(f=0;f<b.length;f++)b[f].renderInterlacedColors();h.restore();if(a&&0<a.length)for(f=0;f<a.length;f++)a[f].renderGrid(),r&&(a[f].createMask(),a[f].renderBreaksBackground());if(e&&0<e.length)for(f=0;f<e.length;f++)e[f].renderGrid(),r&&(e[f].createMask(),e[f].renderBreaksBackground());
...
a.length;f++)a[f].renderStripLinesOfThicknessType("pixel"),a[f].crosshair&&a[f].crosshair.enabled&&(h=!0,d.clearedOverlayedCanvas=a[f].type);if(e&&0<e.length)for(f=0;f<e.length;f++)e[f].renderStripLinesOfThicknessType("pixel"),e[f].crosshair&&e[f].crosshair.enabled&&(h=!0,d.clearedOverlayedCanvas=e[f].type);if(c&&0<c.length)for(f=0;f<c.length;f++)c[f].renderStripLinesOfThicknessType("pixel"),c[f].crosshair&&c[f].crosshair.enabled&&(h=!0,d.clearedOverlayedCanvas=c[f].type);if(b&&0<b.length)for(f=0;f<
b.length;f++)b[f].renderStripLinesOfThicknessType("pixel"),b[f].crosshair&&b[f].crosshair.enabled&&(h=!0,d.clearedOverlayedCanvas=b[f].type);if(h||!h&&d.clearedOverlayedCanvas&&0<=d.clearedOverlayedCanvas.indexOf("axis"))d.resetOverlayedCanvas(),d.renderCrosshairs(null,l(d.sessionVariables.crosshairShownByPixel)||d.sessionVariables.crosshairShownByPixel?!0:d.sessionVariables.crosshairShownByPixel)};B.prototype.calculateStripLinesThicknessInValues=function(){for(var a=0;a<this.stripLines.length;a++)if(null!==
...
Va(e[h].spacing,a,8,d?0.1*a:8,d?0:3)<<0,e[h].size=0>e[h].spacing?0:Math.abs(e[h].spacing/c),this.logarithmic&&(e[h].size=Math.pow(this.logarithmBase,e[h].size))};B.prototype.calculateBreaksInPixels=function(){if(!(this.scaleBreaks&&0>=this.scaleBreaks._appliedBreaks.length)){var a=this.scaleBreaks?this.scaleBreaks._appliedBreaks:[];a.length&&(this.scaleBreaks.firstBreakIndex=this.scaleBreaks.lastBreakIndex=null);for(var e=0;e<a.length&&!(a[e].startValue>this.conversionParameters.maximum);e++)a[e].endValue<
this.conversionParameters.minimum||(l(this.scaleBreaks.firstBreakIndex)&&(this.scaleBreaks.firstBreakIndex=e),a[e].startValue>=this.conversionParameters.minimum&&(a[e].startPixel=this.convertValueToPixel(a[e].startValue),this.scaleBreaks.lastBreakIndex=e),a[e].endValue<=this.conversionParameters.maximum&&(a[e].endPixel=this.convertValueToPixel(a[e].endValue)))}};B.prototype.renderLabelsTicksAndTitle=function(){var a=this,e=!1,c=0,b=0,d=1,h=0;0!==this.labelAngle&&360!==this.labelAngle&&(d=1.2);if("undefined"===
typeof this.options.interval){if("bottom"===this._position||"top"===this._position)if(this.logarithmic&&!this.equidistantInterval&&this.labelAutoFit){for(var c=[],d=0!==this.labelAngle&&360!==this.labelAngle?1:1.2,f,l=this.viewportMaximum,r=this.lineCoordinates.width/Math.log(this.range),k=this._labels.length-1;0<=k;k--){n=this._labels[k];if(n.position<this.viewportMinimum)break;n.position>this.viewportMaximum||!(k===this._labels.length-1||f<Math.log(l/n.position)*r/d)||(c.push(n),l=n.position,f=
n.textBlock.width*Math.abs(Math.cos(Math.PI/180*this.labelAngle))+n.textBlock.height*Math.abs(Math.sin(Math.PI/180*this.labelAngle)))}this._labels=c}else{for(k=0;k<this._labels.length;k++)n=this._labels[k],n.position<this.viewportMinimum||(f=n.textBlock.width*Math.abs(Math.cos(Math.PI/180*this.labelAngle))+n.textBlock.height*Math.abs(Math.sin(Math.PI/180*this.labelAngle)),c+=f);c>this.lineCoordinates.width*d&&this.labelAutoFit&&(e=!0)}if("left"===this._position||"right"===this._position)if(this.logarithmic&&
...
p.y-(("inside"===this.tickPlacement?this.tickLength:0)+n.textBlock.height-n.textBlock.fontSize/2):p.y+("inside"===this.tickPlacement?0:this.tickLength)+n.textBlock.fontSize/2+5):0>this.labelAngle?(p.x=p.x-("inside"===this.labelPlacement?0:n.textBlock.width*Math.cos(Math.PI/180*this.labelAngle))+n.textBlock.topPadding*Math.sin(Math.PI/180*this.labelAngle),p.y="inside"===this.labelPlacement?p.y-("inside"===this.tickPlacement?this.tickLength:0)-(n.textBlock.height-n.textBlock.fontSize/2)*Math.cos(Math.PI/
180*this.labelAngle):p.y+("inside"===this.tickPlacement?0:this.tickLength)+5+Math.abs(n.textBlock.width*Math.sin(Math.PI/180*this.labelAngle)-n.textBlock.fontSize/2*Math.cos(Math.PI/180*this.labelAngle))):(p.x=p.x-("inside"===this.labelPlacement?n.textBlock.width*Math.cos(Math.PI/180*this.labelAngle):0)+n.textBlock.topPadding*Math.sin(Math.PI/180*this.labelAngle),p.y="inside"===this.labelPlacement?p.y-("inside"===this.tickPlacement?this.tickLength:0)-n.textBlock.width*Math.sin(Math.PI/180*this.labelAngle)-
(n.textBlock.height-n.textBlock.fontSize/2)*Math.cos(Math.PI/180*this.labelAngle):p.y+("inside"===this.tickPlacement?0:this.tickLength)+5+Math.abs(n.textBlock.fontSize/2*Math.cos(Math.PI/180*this.labelAngle))),n.textBlock.x=p.x,n.textBlock.y=p.y));"inside"===this.tickPlacement&&this.chart.addEventListener("dataAnimationIterationEnd",function(){for(k=0;k<a._labels.length;k++)if(n=a._labels[k],!(n.position<a.viewportMinimum||n.position>a.viewportMaximum)&&(p=a.getPixelCoordinatesOnAxis(n.position),
...
2-1,this.titleMaxWidth=this._titleTextBlock.maxWidth,this._titleTextBlock.render(!0))}else if("top"===this._position){for(k=0;k<this._labels.length;k++)n=this._labels[k],n.position<this.viewportMinimum||n.position>this.viewportMaximum||(p=this.getPixelCoordinatesOnAxis(n.position),this.tickThickness&&"inside"!=this.tickPlacement&&(this.ctx.lineWidth=this.tickThickness,this.ctx.strokeStyle=this.tickColor,b=1===this.ctx.lineWidth%2?(p.x<<0)+0.5:p.x<<0,this.ctx.beginPath(),this.ctx.moveTo(b,p.y<<0),
this.ctx.lineTo(b,p.y-this.tickLength<<0),this.ctx.stroke()),e&&0!==h++%2&&this.labelAutoFit||(0===n.textBlock.angle?(p.x-=n.textBlock.width/2,p.y="inside"===this.labelPlacement?p.y+this.labelFontSize/2+("inside"===this.tickPlacement?this.tickLength:0)+5:p.y-(("inside"===this.tickPlacement?0:this.tickLength)+n.textBlock.height-n.textBlock.fontSize/2)):0>this.labelAngle?(p.x=p.x-("inside"===this.labelPlacement?n.textBlock.width*Math.cos(Math.PI/180*this.labelAngle):0)+n.textBlock.topPadding*Math.sin(Math.PI/
180*this.labelAngle),p.y="inside"===this.labelPlacement?p.y+("inside"===this.tickPlacement?this.tickLength:0)+5-n.textBlock.width*Math.sin(Math.PI/180*this.labelAngle)+n.textBlock.fontSize/2*Math.cos(Math.PI/180*this.labelAngle):p.y-("inside"===this.tickPlacement?0:this.tickLength)-2-(n.textBlock.height-n.textBlock.fontSize/2)*Math.cos(Math.PI/180*this.labelAngle)):(p.x=p.x-("inside"===this.labelPlacement?0:n.textBlock.width*Math.cos(Math.PI/180*this.labelAngle))+n.textBlock.topPadding*Math.sin(Math.PI/
180*this.labelAngle),p.y="inside"===this.labelPlacement?p.y+("inside"===this.tickPlacement?this.tickLength:0)+5+n.textBlock.fontSize/2*Math.cos(Math.PI/180*this.labelAngle):p.y-("inside"===this.tickPlacement?0:this.tickLength)-2-((n.textBlock.height-n.textBlock.fontSize/2)*Math.cos(Math.PI/180*this.labelAngle)+n.textBlock.width*Math.sin(Math.PI/180*this.labelAngle))),n.textBlock.x=p.x,n.textBlock.y=p.y));"inside"===this.tickPlacement&&this.chart.addEventListener("dataAnimationIterationEnd",function(){for(k=
...
0<this.stripLines.length&&a){var e=this,c,b,d=0,h=0,f=!1;b=!1;for(var r=[],t=[],k=!1,m,d=0;d<this.stripLines.length;d++){var n=this.stripLines[d];n._thicknessType===a&&("pixel"===a&&(n.value<this.viewportMinimum||n.value>this.viewportMaximum||l(n.value)||isNaN(this.range))||"value"===a&&(n.startValue<=this.viewportMinimum&&n.endValue<=this.viewportMinimum||n.startValue>=this.viewportMaximum&&n.endValue>=this.viewportMaximum||l(n.startValue)||l(n.endValue)||isNaN(this.range))||r.push(n))}for(d=0;d<
this._stripLineLabels.length;d++)if(n=this.stripLines[d],c=this._stripLineLabels[d],!(c.position<this.viewportMinimum||c.position>this.viewportMaximum||isNaN(this.range)))if(b=this.getPixelCoordinatesOnAxis(c.position),"outside"===c.stripLine.labelPlacement){n&&(this.ctx.strokeStyle=n.color,this.ctx.lineWidth="pixel"===n._thicknessType?n.thickness:this.tickThickness,l(n.opacity)||"pixel"!==n._thicknessType||(m=n.ctx.globalAlpha,n.ctx.globalAlpha=n.opacity));if("bottom"===this._position){var p=1===
this.ctx.lineWidth%2?(b.x<<0)+0.5:b.x<<0;this.ctx.beginPath();this.ctx.moveTo(p,b.y<<0);this.ctx.lineTo(p,b.y+this.tickLength<<0);this.ctx.stroke();0===this.labelAngle?(b.x-=c.textBlock.width/2,b.y+=this.tickLength+c.textBlock.fontSize/2+5):(b.x-=(0>this.labelAngle?c.textBlock.width*Math.cos(Math.PI/180*this.labelAngle):0)-c.textBlock.topPadding*Math.sin(Math.PI/180*this.labelAngle),b.y+=this.tickLength+5+(0>this.labelAngle?Math.abs(c.textBlock.width*Math.sin(Math.PI/180*this.labelAngle)-c.textBlock.fontSize/
...
c.moveTo(a-d/2,e-d/2),c.lineTo(a+d/2,e+d/2),c.stroke(),c.moveTo(a+d/2,e-d/2),c.lineTo(a-d/2,e+d/2),c.stroke())}},drawMarkers:function(a){for(var e=0;e<a.length;e++){var c=a[e];ba.drawMarker(c.x,c.y,c.ctx,c.type,c.size,c.color,c.borderColor,c.borderThickness)}}};return x}();t.version="v3.12.0 GA";window.CanvasJS&&(t&&!window.CanvasJS.Chart)&&(window.CanvasJS.Chart=t)})();
